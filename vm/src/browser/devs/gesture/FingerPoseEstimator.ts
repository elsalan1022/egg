...
export default class FingerPoseEstimator {
  ...
  getSlopes(point1: string | any[], point2: any[]) {
    ...
    const slopeXY = this.calculateSlope(point1[0], point1[1], point2[0], point2[1]);
    if (point1.length == 2) {
      return slopeXY;
    }

    const slopeYZ = this.calculateSlope(point1[1], point1[2], point2[1], point2[2]);
  ...
  calculateFingerDirection(startPoint: any[], midPoint: any[], endPoint: any[], fingerSlopes: any[]) {
    ...
    const totalAngle = this.getSlopes(calcStartPoint, calcEndPoint);
    const votes = this.angleOrientationAt(totalAngle, this.options.TOTAL_ANGLE_VOTE_POWER);
    voteVertical += votes[0];
    voteDiagonal += votes[1];
    voteHorizontal += votes[2];

    for (const fingerSlope of fingerSlopes) {
      const votes = this.angleOrientationAt(fingerSlope, this.options.SINGLE_ANGLE_VOTE_POWER);
      voteVertical += votes[0];
      voteDiagonal += votes[1];
      voteHorizontal += votes[2];
    }
