...
import { Coords3D, HandPipeline, Prediction } from '@tensorflow-models/handpose/dist//pipeline';

// Load the bounding box detector model.
async function loadHandDetectorModel() {
  const HANDDETECT_MODEL_PATH =
    './assets/models/gesture/handdetector/model.json';
...
async function loadHandPoseModel() {
  const HANDPOSE_MODEL_PATH =
    './assets/models/gesture/handskeleton/model.json';
...
async function loadAnchors() {
  return tf.util
    .fetch(
      './assets/models/gesture/anchors.json')
    .then(d => d.json());
}
